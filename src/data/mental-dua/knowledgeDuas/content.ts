export const knowledgeDuas = {
  id: "knowledgeDuas",

  heading: {
    arabic: "أَدْعِيَةُ الْعِلْمِ",
    malayalam: "വിദ്യക്കും അറിവിനും വേണ്ട ദുആകൾ",
    english: "Knowledge Duas",
  },

  isBox: true,

  content: [
    {
      id: 1,
      // Increase in knowledge (Qur'an)
      arabic: [
        "رَبِّ زِدْنِي عِلْمًا"
      ],
      malayalam: [
        "എന്റെ രക്ഷിതാവേ,",
        "എന്റെ അറിവ് വർദ്ധിപ്പിക്കണമേ."
      ],
      english: [
        "My Lord, increase me in knowledge."
      ],
    },

    {
      id: 2,
      // Beneficial knowledge
      arabic: [
        "اللَّهُمَّ انْفَعْنِي بِمَا عَلَّمْتَنِي",
        "وَعَلِّمْنِي مَا يَنْفَعُنِي",
        "وَزِدْنِي عِلْمًا"
      ],
      malayalam: [
        "അല്ലാഹുമ്മാ, നീ എന്നെ പഠിപ്പിച്ചതിലൂടെ എനിക്ക് പ്രയോജനം നൽകണമേ.",
        "എനിക്ക് പ്രയോജനപ്പെടുന്നതെല്ലാം നീ എന്നെ പഠിപ്പിക്കണമേ.",
        "എന്റെ അറിവ് വർദ്ധിപ്പിക്കണമേ."
      ],
      english: [
        "O Allah, benefit me with what You have taught me,",
        "teach me what will benefit me,",
        "and increase me in knowledge."
      ],
    },

    {
      id: 3,
      // Understanding & wisdom
      arabic: [
        "اللَّهُمَّ فَقِّهْنِي فِي الدِّينِ",
        "وَعَلِّمْنِي التَّأْوِيلَ"
      ],
      malayalam: [
        "അല്ലാഹുമ്മാ,",
        "എനിക്ക് ദീനിൽ ബോധം നൽകണമേ.",
        "വിവേകവും മനസ്സിലാക്കലും പഠിപ്പിക്കണമേ."
      ],
      english: [
        "O Allah, grant me understanding of the religion,",
        "and teach me interpretation and wisdom."
      ],
    },

    {
      id: 4,
      // Knowledge with action
      arabic: [
        "اللَّهُمَّ إِنِّي أَعُوذُ بِكَ",
        "مِنْ عِلْمٍ لَا يَنْفَعُ"
      ],
      malayalam: [
        "അല്ലാഹുമ്മാ,",
        "എനിക്ക് പ്രയോജനമില്ലാത്ത അറിവിൽ നിന്ന്",
        "ഞാൻ നിന്നിൽ ശരണം തേടുന്നു."
      ],
      english: [
        "O Allah, I seek refuge in You",
        "from knowledge that does not benefit."
      ],
    },

    {
      id: 5,
      // Success in studies
      arabic: [
        "رَبِّ اشْرَحْ لِي صَدْرِي",
        "وَيَسِّرْ لِي أَمْرِي"
      ],
      malayalam: [
        "എന്റെ രക്ഷിതാവേ,",
        "എന്റെ ഹൃദയം നീ തുറക്കണമേ.",
        "എന്റെ കാര്യങ്ങൾ എളുപ്പമാക്കണമേ."
      ],
      english: [
        "My Lord, expand for me my chest,",
        "and ease for me my task."
      ],
    },
  ],
};
