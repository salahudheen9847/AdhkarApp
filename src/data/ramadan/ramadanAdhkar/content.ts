export const ramadanAdhkar = {
  id: "ramadanAdhkar",

  // 👉 Detail screen heading
  heading: {
    arabic: "أذكار رمضان",
    malayalam: "റമളാൻ അദ്‌കാറുകൾ",
    english: "Ramadan Adhkar",
  },

  content: [
    {
      id: 1,
      isHeading: true,
      text: "نِيَّةُ الصِّيَامِ لِكُلِّ لَيْلَةٍ مِنْ رَمَضَانَ",
      malayalam: "റമളാനിലെ ഓരോ രാത്രിയും നിയ്യത്ത് ചെയ്യേണ്ടത്",
      english: "Intention (Niyyah) to be made every night of Ramadan",
      start: 0,
      end: 0,
    },

    {
      id: 2,
      isBox: false,
      text:
        "نَوَيْتُ صَوْمَ غَدٍ عَنْ أَدَاءِ فَرْضِ رَمَضَانِ " +
        "هَذِهِ السَّنَةِ لِلَّهِ تَعَالَى.",
      malayalam:
        "റമളാനിലെ ഓരോ രാവിലും നോമ്പിനുള്ള നിയ്യം: നൊവൈത്തു സൗമ ഗദിന് അൻ അദാഇ ഫർദി റമദാനി ഹാഝിഹിസ്സനതി ലില്ലാഹി തഅാലാ.",
      english:
        "I intend to fast tomorrow to fulfill the obligatory fast of this year’s Ramadan for Allah Most High.",
      start: 0,
      end: 8,
    },

    {
      id: 3,
      isHeading: true,
      text: "دُعَاءُ الْإِفْطَارِ",
      malayalam: "നോമ്പ് തുറക്കുമ്പോൾ ചൊല്ലേണ്ട ദുആ",
      english: "Dua to be recited at the time of breaking the fast",
      start: 0,
      end: 0,
    },

    {
      id: 4,
      isBox: false,
      text:
        "اللَّهُمَّ لَكَ صُمْتُ وَعَلَى رِزْقِكَ أَفْطَرْتُ، " +
        "ذَهَبَ الظَّمَأُ وَابْتَلَّتِ الْعُرُوقُ " +
        "وَثَبَتَ الْأَجْرُ إِنْ شَاءَ الله.",
      malayalam:
        "നോമ്പ് തുറക്കുമ്പോൾ: അല്ലാഹുമ്മ ലക സുമ്തു വ അലാ റിസ്കിക അഫ്തർതു. " +
        "ദഹബഴ്‌ ളമഅ് വബ്‌തല്ലതിൽ ഉരുൿ വ സബതൽ അജ്രു ഇൻ ഷാഅല്ലാഹ്.",
      english:
        "O Allah, for You I have fasted and with Your provision I break my fast. " +
        "The thirst is gone, the veins are moistened, and the reward is assured, if Allah wills.",
      start: 8,
      end: 23,
    },

    {
      id: 5,
      isHeading: true,
      text: "مِمَّا يُقَالُ فِي رَمَضَانَ دَائِمًا",
      malayalam: "റമളാനിൽ എപ്പോഴും ചൊല്ലാവുന്ന ദുആ",
      english: "Supplications to be recited frequently during Ramadan",
      start: 0,
      end: 0,
    },

    {
      id: 6,
      isBox: false,
      text:
        "أَشْهَدُ أَنْ لَا إِلَهَ إِلَّا اللَّهُ " +
        "أَسْتَغْفِرُ اللهَ وَأَسْأَلُكَ الْجَنَّةَ " +
        "وَأَعُوذُ بِكَ مِنَ النَّارِ.",
      malayalam:
        "അശ്‌ഹദു അൻ ലാ ഇലാഹ ഇല്ലല്ലാഹ്. " +
        "അസ്തഗ്ഫിരുല്ലാഹ്, വ അസ്അലുകൽ ജന്നത, വ അഊദു ബിക മിനന്നാർ.",
      english:
        "I bear witness that there is no god but Allah. " +
        "I seek Allah’s forgiveness, ask You for Paradise, and seek refuge in You from the Fire.",
      start: 23,
      end: 31,
    },

    {
      id: 7,
      isHeading: true,
      text: "عَشْرَةُ الرَّحْمَةِ",
      malayalam: "ഒന്നാം പത്ത് (റഹ്മ)",
      english: "The first ten days (Days of Mercy)",
      start: 0,
      end: 0,
    },

    {
      id: 8,
      isBox: false,
      text: "اللَّهُمَّ ارْحَمْنِي يَا أَرْحَمَ الرَّاحِمِينَ",
      malayalam:
        "ഒന്നാം പത്ത് (റഹ്മ): അല്ലാഹുമ്മിർഹംനി യാ അർഹമർ റാഹിമീൻ.",
      english:
        "O Allah, have mercy on me, O Most Merciful of the merciful.",
      start: 31,
      end: 35,
    },

    {
      id: 9,
      isHeading: true,
      text: "عَشْرَةُ الْمَغْفِرَةِ",
      malayalam: "രണ്ടാം പത്ത് (മഗ്ഫിര)",
      english: "The second ten days (Days of Forgiveness)",
      start: 0,
      end: 0,
    },

    {
      id: 10,
      isBox: false,
      text: "اللَّهُمَّ اغْفِرْ لِي ذُنُوبِي يَا رَبَّ الْعَالَمِينَ",
      malayalam:
        "രണ്ടാം പത്ത് (മഗ്ഫിര): അല്ലാഹുമ്മഗ്‌ഫിർലി ധുനൂബി യാ റബ്‌ബൽ ആലമീൻ.",
      english:
        "O Allah, forgive my sins, O Lord of the worlds.",
      start: 35,
      end: 40,
    },

    {
      id: 11,
      isHeading: true,
      text: "عَشْرَةُ النَّجَاةِ",
      malayalam: "മൂന്നാം പത്ത് (നജാത്ത്)",
      english: "The third ten days (Days of Salvation)",
      start: 0,
      end: 0,
    },

    {
      id: 12,
      isBox: false,
      text:
        "اللَّهُمَّ إِنَّكَ عَفُوٌّ تُحِبُّ الْعَفْوَ فَاعْفُ عَنِّي، " +
        "اللَّهُمَّ أَعْتِقْنِي مِنَ النَّارِ " +
        "وَأَدْخِلْنِي الْجَنَّةَ يَا رَبَّ الْعَالَمِينَ.",
      malayalam:
        "അല്ലാഹുമ്മ ഇന്നക അഫുവ്വുൻ തുഹിബ്ബുൽ അഫ്വ ഫഅ്ഫു അണ്ണി. " +
        "അല്ലാഹുമ്മ അഅ്‌തിഖ്‌നി മിനന്നാർ വ അദ്‌ഖിൽനിൽ ജന്നത യാ റബ്‌ബൽ ആലമീൻ.",
      english:
        "O Allah, You are Pardoning and love to pardon, so pardon me. " +
        "Free me from the Fire and admit me to Paradise, O Lord of the worlds.",
      start: 40,
      end: 50,
    },
  ],
} as const;
